...
function sortCol() {   // 对获取的一列排序
	...
	if (i%my.thnum[my.s] == my.ind) {
		...
		// if (!isNaN(parseFloat(my.column[j]))) my.column[j] = parseFloat(my.column[j]);
		++j;
	}
	if (my.Mode[my.s][my.ind]) my.column.sort(function(a, b) { return a < b ? 1 : -1; });
...
function change() {   // 将排序后的列表更新
    ...
    $("#table-" + my.s + " tbody"+" tr:odd").each(function() {
